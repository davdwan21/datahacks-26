...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (displacementProgress < 0.3) {
      const prog = displacementProgress / 0.3
      screenX = controller.lerp(spiralPos.x, spiralPos.x + this.dx * 0.3, prog)
      screenY = controller.lerp(spiralPos.y, spiralPos.y + this.dy * 0.3, prog)
    } else if (displacementProgress < 0.7) {
    ...
    // Access private fields via type assertion for projection
    const ctrl = controller as unknown as { cameraZ: number; viewZoom: number }
