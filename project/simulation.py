...
def build_result(env, agents, ticks, policy):
    return {
...
def upload_tick_to_s3(tick_num: int, env: dict, agents: dict, policy: str, bucket: str, prefix: str):
    data = {
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "year": tick_num,
        "policy": policy,
    ...
    key = f"{prefix}/year_{tick_num:04d}.json"
    upload_json_to_s3(data, bucket, key)
...
def run_simulation(env: dict, agents: dict, ticks: int = 5, verbose: bool = False,
    ...
    for tick_num in range(1, ticks + 1):
        ...
        if output_bucket:
            upload_tick_to_s3(tick_num, env, agents, policy, output_bucket, output_prefix)
...
if __name__ == "__main__":
    ...
    output_prefix = os.environ.get("OUTPUT_PREFIX", "simulation-output")
    ...
    final_env, final_agents = run_simulation(
        env, agents,
        ticks=args.ticks,
    ...
    result = build_result(final_env, final_agents, args.ticks, args.policy)

    if output_bucket:
        key = f"{output_prefix}/result.json"
        upload_json_to_s3(result, output_bucket, key)
        print(f"Uploaded final summary to s3://{output_bucket}/{key}")
    else:
